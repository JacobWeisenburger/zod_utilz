...
test('coerce( z.string() )', () => {
	...
	expect(schema.parse({ foo: 'foo', bar: 'bar' })).toBe(
		'{"foo":"foo","bar":"bar"}',
	)
	expect(schema.parse([])).toBe('[]')
	expect(schema.parse(['foo'])).toBe('["foo"]')
...
test('coerce( z.any().array() )', () => {
	...
	expect(schema.parse({ foo: 'foo', bar: 'bar' })).toEqual([
		{ foo: 'foo', bar: 'bar' },
	])
	expect(schema.parse([])).toEqual([])
	expect(schema.parse(['foo'])).toEqual(['foo'])
...
test('coerce( z.string().array() )', () => {
	...
	expect(schema.parse({ foo: 'foo', bar: 'bar' })).toMatchObject([
		'{"foo":"foo","bar":"bar"}',
	])
	expect(schema.parse([])).toMatchObject([])
	expect(schema.parse(['foo'])).toMatchObject(['foo'])
...
test('README Example: z.bigint()', () => {
	...
	expect(bigintSchema.safeParse('foo').error?.issues[0].message).toBe(
...
test('README Example: z.number().array()', () => {
	...
	expect(numberArraySchema.safeParse('foo').error?.issues[0].message).toBe(
