...
test('strict', () => {
	...
	).toMatchObject(["Unrecognized key(s) in object: 'extraKey'"])
})
...
test('README Example', () => {
	...
	).toMatchObject({ string: 'foo', number: 42, boolean: false })
...
test('README Example', () => {
...

	{
		...
		expect(schema.safeParse(formData).data).toMatchObject({
			string: 'foo',
			number: 42,
			boolean: false,
			file,
